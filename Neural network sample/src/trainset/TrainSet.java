...
public class TrainSet {
    ...
    public void addData(double[] in, double[] expected) {// add data set on the training set
        if(in.length != INPUT_SIZE || expected.length != OUTPUT_SIZE) return;
        data.add(new double[][]{in, expected});
    }

    public TrainSet extractBatch(int size) {// extract a specific number of data sets from the training set
    ...
    public int size() { // return the size or how many sets are there
        return data.size();
    }
